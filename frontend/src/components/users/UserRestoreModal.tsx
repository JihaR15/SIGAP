...
export function UserRestoreModal({
  ...
  return (
    <div
      ...
      onMouseDown={(e) => e.target === e.currentTarget && !isProcessing && onClose()}
    >
      <div className="bg-white rounded-xl shadow-lg w-full max-w-sm border border-gray-200 text-center p-6 animate-in fade-in zoom-in duration-200">
        ...
        <p className="text-sm text-gray-500 mb-6">
          Pengguna ini akan mendapatkan kembali akses login ke sistem SIGAP sesuai dengan hak akses perannya.
        </p>
        <div className="flex justify-center gap-3">
          ...
          <button
            onClick={onConfirm}
            disabled={isProcessing}
            className="px-4 py-2 text-sm font-semibold text-white bg-green-600 hover:bg-green-700 rounded-lg disabled:opacity-50 transition-colors shadow-sm"
          >
